...
- **Cost.** Default budget is $25. At ~$0.03 per query per provider × 3 providers × 3 repetitions × 50 queries, a single full measurement is roughly $13-15 — so expect ~2 full score measurements in one budget. Most runs sample queries via `SHELF_QUERIES_PER_MEASUREMENT`.
